...
int cmd_state;
double strideTarget, adjustTarget, strideTarget_mm, adjustTarget_mm;
double difference;
...
void send_485() { // This function to send out 485 com to the AGV
  switch (cmd_state) {
    case 0: // idle
      ...
      break;
    case 1: // forward
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(fwd[i]);
      cmd_state = 0;
      break;
    case 2: // backward
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(bkwd[i]);
      cmd_state = 0;
      break;
    case 3: // ccw
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(ccw[i]);
      cmd_state = 0;
      break;
    case 4: // cw
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(cw[i]);
      cmd_state = 0;
      break;
    case 5: // slower
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(slow[i]);
      cmd_state = 0;
      break;
    case 6: // faster
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(fast[i]);
      cmd_state = 0;
      break;
    case 7: // left
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(left[i]);
      cmd_state = 0;
      break;
    case 8: // right
      for (int j = 0; j < group; j++) for (int i = 0; i < 11; i++) rs485.write(right[i]);
      cmd_state = 0;
      break;  
    default:
      break;
...
void process_terminal(int incomingByte) { // This function to process the incoming terminal command
  if (incomingByte == 32) cmd_state = 0; // space (idle)
    ...
    adjustTarget_mm = 300; //adjustTarget_mm = Serial.parseInt();
...
void align() {
    alg_timeout = millis();
    if (abs(rUsonicRead - lUsonicRead) >= 6) {
        int err = abs(rUsonicRead - lUsonicRead);
        P_alg = 12 * err;
        D_alg = ((2 * 5) / (0.05 + 2 * 0.5)) * (err - prev_e_alg) - ((0.05 - 2 * 0.5) / (0.05 + 2 * 0.5)) * D_alg;
        ...
        group = PID/70;
        if (rUsonicRead > lUsonicRead) {
            cmd_state = 3;
        }
        else if (rUsonicRead < lUsonicRead) {
            cmd_state = 4;
        }
    }
    else {
    ...
    if (cnt_alg >= 20 && abs(rUsonicRead - lUsonicRead) <= 6) {
        align_flag = false;
        cnt_alg = 0;
        group = 1;
        D_alg = 0;
    }
    else if (millis() - alg_timeout > 10000 && abs(rUsonicRead - lUsonicRead) <= 10) {
        align_flag = false;
        cnt_alg = 0;
        group = 1;
        D_alg = 0;
